#!/usr/bin/env python3
"""
IV-GICP Ablation Study: C1 / C2 / C3 contributions (paper definition).
...
from iv_gicp.config_loader import get_datasets_config, get_pipeline_config
...
def ate_rmse(poses_est, poses_gt):
    ...
    R = Vt.T @ D @ U.T
    t = mu_g - R @ mu_e
    errs = np.array([np.linalg.norm(R @ p[:3, 3] + t - g[:3, 3]) for p, g in zip(poses_est[:n], poses_gt[:n])])
    return float(np.sqrt(np.mean(errs**2)))


def rpe_rmse(poses_est, poses_gt, delta=1):
    """Relative pose error (translation) over delta frames. No global alignment."""
    n = min(len(poses_est), len(poses_gt))
    if n < delta + 1:
        return float("nan")
    errs = []
    for i in range(n - delta):
        # rel_est = T_est_i^{-1} @ T_est_{i+delta}
        P0_e = np.linalg.inv(poses_est[i]) @ poses_est[i + delta]
        P0_g = np.linalg.inv(poses_gt[i]) @ poses_gt[i + delta]
        errs.append(np.linalg.norm(P0_e[:3, 3] - P0_g[:3, 3]))
...
_VLP16_ELEVATIONS = np.deg2rad([-15, 1, -13, 3, -11, 5, -9, 7, -7, 9, -5, 11, -3, 13, -1, 15])
...
def load_subt(max_frames=300):
    ...
    for t, msgdef in [
        ...
        ("velodyne_msgs/msg/VelodyneScan", "std_msgs/Header header\nvelodyne_msgs/VelodynePacket[] packets\n"),
    ]:
...
# ── GEODE Urban Tunnel data loading ───────────────────────────────────────────
...
def load_geode_tunnel(max_frames=300, seq="01"):
    from rosbags.rosbag1 import Reader
    from rosbags.typesys import Stores, get_typestore
    from scipy.spatial.transform import Rotation

    bag_path = GEODE_ROOT / f"sensor_data/Urban_tunnel/Urban_Tunnel{seq}/Urban_Tunnel{seq}.bag"
    gt_path  = GEODE_ROOT / f"groundtruth/Urban_tunnel/Urban_Tunnel{seq}.txt"

    # GT: timestamp x y z qx qy qz qw
    ...
    for i in range(len(gt_ts)):
        R = Rotation.from_quat(gt_quat[i]).as_matrix()
        T = np.eye(4)
        T[:3, :3] = R
        T[:3, 3]  = gt_txyz[i]
        if T0_inv is None:
    ...
    with Reader(bag_path) as bag:
        conns = [c for c in bag.connections if c.topic == "/velodyne_points"]
    ...
    # Match GT by nearest timestamp
    frame_ts = np.array([f[0] for f in frames])
    idx = np.clip(np.searchsorted(gt_ts, frame_ts), 0, len(gt_ts) - 1)
    matched_gt = [gt_poses[i] for i in idx]
...
def run_config(
    ...
    rpe = rpe_rmse(abs_poses, poses_gt, delta=1)
    hz = 1000.0 / np.mean(times[1:]) if len(times) > 1 else 0.0
    print(f"  [{label}]  ATE={ate:.5f}m  RPE(Δ=1)={rpe:.5f}m  {hz:.1f}Hz")
    return ate, rpe, hz, abs_poses
...
def _build_datasets():
    """Build DATASETS from config/datasets.yaml; fallback to defaults if missing."""
    raw = get_datasets_config()
    if not raw:
        return {
            "kitti": {"loader": load_kitti, "loader_kw": {}, "alpha": 0.1, "params": dict(
            ...
            "subt": {"loader": load_subt, "loader_kw": {}, "alpha": 0.25, "params": dict(
            ...
            "metro": {"loader": load_metro, "loader_kw": {}, "alpha": 0.5, "params": dict(
            ...
            "geode": {"loader": load_geode_tunnel, "loader_kw": {"seq": "01"}, "alpha": 0.3, "params": dict(
            ...
            ), "label": "GEODE Urban_Tunnel01 (urban tunnel)"},
        }
    out = {}
