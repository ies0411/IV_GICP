...
- **제안 아이디어:** **Octree 기반의 계층적 가우시안 혼합 모델 (Hierarchical GMM)**
  - 각 노드(복셀)는 하나의 가우시안 분포 $\\mathcal{N}(\\mu, \\Sigma)$를 가집니다.
  - **분할 기준 (Splitting Criterion):** **섀넌 엔트로피 (Shannon Entropy)** $H(X)$를 활용합니다.
    $$H(X) \= \\frac{1}{2} \\ln |(2\\pi e)^k \\Sigma|$$
  - **알고리즘:**
    1. 초기 큰 복셀(Leaf node)의 공분산 행렬식 $|\\Sigma|$ (즉, 불확실성/엔트로피)을 계산합니다.
    2. 이 값이 임계값(Threshold)을 넘으면, 해당 복셀 내의 점들의 분포가 \*\*"너무 복잡하거나 평면이 아니다"\*\*라고 판단하여 8개로 쪼갭니다(Sub-divide).
    3. **Key Innovation:** 여기에 **Intensity Variance**도 포함합니다. 평평한 바닥이라 기하학적 엔트로피는 낮아도, **차선(Paint)이 있어 Intensity 분산이 높으면** 쪼갭니다. 이렇게 하면 중요 특징점만 정밀하게 타격하는 Voxel Map이 완성됩니다.
...
- **기존 FORM:** Pose $T\_{old}$가 $T\_{new}$로 바뀌면, 점 $p$를 $T\_{new} T\_{old}^{-1} p$로 이동. (점 10만 개면 10만 번 연산)
- **제안 아이디어:** 점을 옮기지 말고, **복셀의 통계치(Mean $\\mu$, Covariance $\\Sigma$)만 업데이트**합니다.
  - Factor Graph 최적화를 통해 궤적 변화량 $\\Delta T$가 산출됩니다.
  - **Lie Theory 기반 전파 (Propagation):**
    - 평균 업데이트: $\\mu\_{new} \= \\Delta T \\mu\_{old}$
    - 공분산 업데이트: $\\Sigma\_{new} \\approx R(\\Delta T) \\Sigma\_{old} R(\\Delta T)^T$
  - **효과:** 수백만 개의 점을 건드리는 대신, 수천 개의 복셀 파라미터(행렬)만 업데이트하므로 FORM 대비 계산 비용이 1/100 수준으로 감소합니다.
...
| 항목          | 내용                                                                                          |
| ------------- | --------------------------------------------------------------------------------------------- |
| **Platform**  | ROS 2 / Python 3.10+                                                                          |
| **Datasets**  | KITTI (corridor-like), Indoor tunnel/corridor sequences, Custom tunnel                        |
| **Baselines** | VGICP, GICP, GenZ-ICP, NDT, FORM (map refinement)                                             |
| **Metrics**   | ATE (Absolute Trajectory Error), RPE (Relative Pose Error), Map update time (ms), Voxel count |

### 5.2 Experimental Scenarios

1. **Geometric Degeneracy (터널/복도):**
   - VGICP/GenZ-ICP drift 발생 구간 식별
   - IV-GICP: Intensity (차선, 벽면 텍스처)로 drift 억제
   - 정량: ATE 비교

2. **Adaptive Voxelization:**
   - 평지 vs 복잡 구조물에서 복셀 크기 분포 시각화 (Fig)
   - 복셀 수 대비 정합 정확도 트레이드오프

3. **Map Refinement Efficiency:**
   - FORM vs Distribution Propagation: Pose 1개 변경 시 업데이트 시간
   - 점 수 10만/100만 확장 시 차이

### 5.3 Ablation Study

| Ablation         | Adaptive | Intensity | Dist. Prop. | ATE (m) |
| ---------------- | -------- | --------- | ----------- | ------- |
| VGICP (baseline) | ✗        | ✗         | ✗           | -       |
| + Adaptive only  | ✓        | ✗         | ✗           | -       |
| + Intensity only | ✗        | ✓         | ✗           | -       |
...
| **IV-GICP**                  | Voxel(mean_4d, cov_4d), corr_pairs           | build_photometric_cov(), solve_gauss_newton() |
| **Distribution Propagation** | voxel_map: dict[voxel_id, (μ, Σ)]            | propagate_delta_T(ΔT, voxel_map)              |

**파이프라인:** PointCloud → AdaptiveVoxelMap → IV-GICP registration → (optional) FactorGraph → RetroactivePropagation → RefinedMap
